-- =========================================================
-- CONTRACT REGISTRY — MIGRATION 011
-- Plan presets + entitlement bundles + hosted tier law
-- Depends on:
--   001..010
-- Rooted-style posture:
--   plans are explicit
--   entitlements derive from plan bundles
--   limits are stored, not implied
--   hosted tier law is deterministic
-- =========================================================
...
create table if not exists billing.plan_definitions (
  ...
  constraint plan_definitions_key_chk
    check (length(btrim(plan_key)) > 0),

  constraint plan_definitions_title_chk
    check (length(btrim(title)) > 0)
);

create index if not exists plan_definitions_active_idx
...
create index if not exists plan_entitlement_presets_entitlement_idx
...
create table if not exists billing.plan_limit_presets (
  ...
  constraint plan_limit_presets_limit_key_chk
    check (length(btrim(limit_key)) > 0),

  constraint plan_limit_presets_limit_int_chk
    check (limit_int >= 0)
);
...
-- ---------------------------------------------------------
-- 5) Helper: effective entitlement from plan preset
-- ---------------------------------------------------------
...
--    Explicit upsert helper for hosted plan application
-- ---------------------------------------------------------
