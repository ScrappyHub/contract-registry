...
function CanonJsonFileToBytes([string]$jsonPath){
  ...
  # ConvertFrom-Json preserves structure; then we re-serialize canonically
  $obj = $raw | ConvertFrom-Json
  return (CanonJsonBytes $obj)
}

# Resolve signing key if signing is enabled.
...
$PacketRoot = Join-Path $OutDir ("contract_release_" + $safeRef + "_" + $utc)
...
# 2) payload/commit.payload.json + payload/commit_hash.txt (commit hash = sha256(canonical bytes(commit.payload.json)))
...
# 3) manifest.json (NO packet_id) — PacketId derived from canonical bytes(manifest-without-id)
$ManifestObj = [ordered]@{
...
# 5) Optional detached signature (sshsig) over packet_id.txt
$SigPath = ""
if (-not $NoSign) {
  ...
  # ssh-keygen -Y sign writes <file>.sig by default; we control output by copying/renaming.
