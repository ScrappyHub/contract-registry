...
# ---------------------------------------------------------
# 2) Write resolver script (OVERWRITE WITH KNOWN-GOOD)
# ---------------------------------------------------------
...
[void]$R.Add("  $canonList=New-Object System.Collections.Generic.List[string]; foreach($k in ($canon.Keys | Sort-Object)){ $h=Sha256HexFile $canon[$k]; [void]$canonList.Add(($h + ""  "" + $k)) }")
[void]$R.Add("  $overList=New-Object System.Collections.Generic.List[string];  foreach($k in ($over.Keys  | Sort-Object)){ $h=Sha256HexFile $over[$k];  [void]$overList.Add(($h + ""  "" + $k)) }")
...
# ---------------------------------------------------------
# 3) Wire Tier-0 runner (idempotent via sentinel)
# ---------------------------------------------------------
...
if($raw -like ("*" + $runnerSentinel + "*")){
  ...
  $bkDir = Join-Path $Scratch ("backups\wire_effective_sets_v2_" + [DateTime]::UtcNow.ToString("yyyyMMdd_HHmmss"))
...
Write-Host "PATCH_ALL_OK_V2" -ForegroundColor Green
