...
$resolverMarker = "contract_registry_resolve_effective_sets_v1.ps1"

# Remove any old effective-sets blocks:
# - sentinel block
# - any resolver-marker block that may have been inserted without sentinel
...
# Find insertion point immediately before TIER0_OK line
$ins = -1
for($i=0;$i -lt $out1.Count;$i++){
  if($out1[$i] -match 'Write-Host\s*\("TIER0_OK: receipt="\s*\+\s*\$Receipt\)'){
    $ins = $i
    break
  }
}
if($ins -lt 0){
  for($i=0;$i -lt $out1.Count;$i++){
    if($out1[$i] -match 'TIER0_OK: receipt='){
      $ins = $i
      break
    }
  }
}
if($ins -lt 0){ Die "PATCH_FAIL_NO_TIER0_OK_ANCHOR" }
...
[void]$blk.Add('if([string]::IsNullOrWhiteSpace($Receipt)){ Die "EFFECTIVE_SETS_RECEIPT_VAR_EMPTY" }')
[void]$blk.Add('$ReceiptPath = $Receipt')
...
$bkDir = Join-Path $Scratch ("backups\replace_effective_sets_block_use_receipt_" + [DateTime]::UtcNow.ToString("yyyyMMdd_HHmmss"))
...
Write-Host ("PATCH_OK_REPLACED_EFFECTIVE_SETS_BLOCK_USE_RECEIPT: " + $Runner) -ForegroundColor Green
