...
# ---------------------------------------------------------
# 2) OVERWRITE resolver script (no nested generator quoting)
# ---------------------------------------------------------
$Resolve = Join-Path $Scripts "contract_registry_resolve_effective_sets_v1.ps1"
$R = New-Object System.Collections.Generic.List[string]
[void]$R.Add("# CONTRACT_REGISTRY_EFFECTIVE_SETS_RESOLVER_V1")
[void]$R.Add("param([Parameter(Mandatory=$true)][string]$RepoRoot,[Parameter(Mandatory=$true)][string]$OutDir,[Parameter()][switch]$AllowOverrides)")
...
# ---------------------------------------------------------
# 3) Wire Tier-0 runner (same sentinel you already use)
# ---------------------------------------------------------
...
if($raw -like ("*" + $runnerSentinel + "*")){ Write-Host ("OK: runner already wired: " + $Runner) -ForegroundColor DarkGray } else {
  ...
  $bkDir = Join-Path $Scratch ("backups\wire_effective_sets_v4_" + [DateTime]::UtcNow.ToString("yyyyMMdd_HHmmss"))
...
Write-Host "PATCH_ALL_OK_V4" -ForegroundColor Green
