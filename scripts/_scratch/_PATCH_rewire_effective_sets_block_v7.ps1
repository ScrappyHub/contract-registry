...
# Remove any existing sentinel block: sentinel line through next blank line (inclusive)
...
# Insert BEFORE the first line that contains "TIER0_OK:" (late in runner, receipt exists)
$ins = -1
for($i=0;$i -lt $out1.Count;$i++){
  if($out1[$i] -match 'TIER0_OK:'){
    $ins = $i
    break
  }
}
...
# Build block that does NOT assume $RcptDir/$ReceiptPath names.
# It discovers the receipt path from common runner variable names via Get-Variable (StrictMode-safe).
...
[void]$blk.Add('# StrictMode-safe discovery of receipt path variable (runner may not use $ReceiptPath/$RcptDir).')
...
$bkDir = Join-Path $Scratch ("backups\rewire_effective_sets_v7_" + [DateTime]::UtcNow.ToString("yyyyMMdd_HHmmss"))
...
Write-Host ("PATCH_OK_REWIRE_EFFECTIVE_SETS_V7: " + $Runner) -ForegroundColor Green
