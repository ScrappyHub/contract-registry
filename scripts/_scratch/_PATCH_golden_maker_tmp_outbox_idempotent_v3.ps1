...
$sentinel = "CONTRACT_REGISTRY_GOLDEN_TMP_OUTBOX_IDEMPOTENT_V3"
if($raw -like ("*" + $sentinel + "*")){ Ok "PATCH_ALREADY_PRESENT"; return }

# Anchor: inject immediately after the first Die function line (literal match).
...
$bkDir = Join-Path (Join-Path (Join-Path $RepoRoot "scripts") "_scratch") ("backups\golden_vectors_fix_v3_" + [DateTime]::UtcNow.ToString("yyyyMMdd_HHmmss"))
