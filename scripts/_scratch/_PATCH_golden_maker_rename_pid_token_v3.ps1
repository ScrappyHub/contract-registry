...
$sentinel = "CONTRACT_REGISTRY_PATCH_RENAME_PID_TOKEN_V3"
if($raw -like ("*" + $sentinel + "*")){ Ok "PATCH_ALREADY_PRESENT"; return }

# Replace bare identifier PID with ChildPid (whole-word only).
$patched = $raw
$rx2 = New-Object System.Text.RegularExpressions.Regex('(?<![A-Za-z0-9_])PID(?![A-Za-z0-9_])')
$patched = $rx2.Replace($patched, { param($m) 'ChildPid' })

# Stamp sentinel at top
$patched = ("# " + $sentinel + "`n") + $patched

# Backup then write
$bkDir = Join-Path (Join-Path (Join-Path $RepoRoot "scripts") "_scratch") ("backups\rename_pid_token_v3_" + [DateTime]::UtcNow.ToString("yyyyMMdd_HHmmss"))
...
ParseGateFile $T
Ok "PATCH_OK_RENAME_PID_TOKEN_V3"
