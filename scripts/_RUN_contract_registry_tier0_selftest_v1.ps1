...
function ReadTextUtf8NoBom([string]$p){ RequireFile $p; return [IO.File]::ReadAllText($p,[Text.UTF8Encoding]::new($false)) }
...
$BuilderV1  = Join-Path $ScriptsDir "contract_registry_make_release_packet_v1.ps1"
$BuilderV11 = Join-Path $ScriptsDir "contract_registry_make_release_packet_v1_1.ps1"
...
RequireFile $Golden
if (Test-Path -LiteralPath $BuilderV1  -PathType Leaf)  { RequireFile $BuilderV1 }
if (Test-Path -LiteralPath $BuilderV11 -PathType Leaf) { RequireFile $BuilderV11 }

# Parse-gate all pipeline scripts we depend on
ParseGateFile $Verifier
ParseGateFile $Selftest
ParseGateFile $Golden
if (Test-Path -LiteralPath $BuilderV1  -PathType Leaf)  { ParseGateFile $BuilderV1 }
if (Test-Path -LiteralPath $BuilderV11 -PathType Leaf) { ParseGateFile $BuilderV11 }
Write-Host "PARSE_GATES_OK: contract-registry scripts" -ForegroundColor Green
...
if (Test-Path -LiteralPath $BuilderV1 -PathType Leaf)  { [void]$rc.Add("scripts/builder_v1_sha256: " + (Sha256HexFile $BuilderV1)) }
if (Test-Path -LiteralPath $BuilderV11 -PathType Leaf) { [void]$rc.Add("scripts/builder_v1_1_sha256: " + (Sha256HexFile $BuilderV11)) }
